...
mod tests {
    ...
    use crate::tui::store::{
        ClosedTaskVisibility, TaskOrder, TaskProjectionOrigin, TaskQuery, TaskScope,
    };
    ...
    async fn collapsed_epic_rows_show_outcomes_signals_and_subtree_activity() {
        ...
        let preview = preview::task_preview_lines(&store.tasks[0], 120, 12)
            .into_iter()
    ...
    async fn empty_epic_row_uses_standard_placeholders() {
        ...
        let preview = preview::task_preview_lines(&store.tasks[0], 100, 12)
            .into_iter()
            .map(|line| line.to_string())
            .collect::<Vec<_>>()
            .join("\n");
    ...
    fn deleted_row_marks_metadata_column_and_keeps_status() {
        ...
        assert!(
            super::preview::task_preview_fields_line(&item)
                .to_string()
    ...
    fn recurring_rows_and_preview_show_series_context() {
        ...
        let preview = preview::task_preview_lines(&item, 80, 20)
            .into_iter()
            .map(|line| line.to_string())
            .collect::<Vec<_>>()
            .join("\n");
